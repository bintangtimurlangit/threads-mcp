...
**21 tools.** Posts are identified by a full `url` **or** `handle` + `code` (the shortcode in `.../@user/post/CODE`).
...
| -------------------------------------------------------------------------------------------------------------------------- | :-------: | :--------: | :---------: |
| `whoami`, `get_profile`, `get_user_threads`, `get_thread`, `get_thread_replies`, `get_timeline`, `search`, `get_followers` |     ✓     |     ✓      |      –      |
